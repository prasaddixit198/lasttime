...
IdentityFile ~/.ssh/id_rsa
  ...
  ** Clone repository at Gitbash **
    ...
    * `pwd` -- Check and verify that the current working directory is C:\users\Hvuser/workspace
    * `git clone ssh://git-codecommit.us-east-2.amazonaws.com/v1/repos/ftp87 `
    * `cd ftp87`
    * `git status`
  * open Visual Studio (VS) Code
    * Open folder c:\users\Hvuser\workspace\ftp87
...
# Workshop material - Day #2 - MySQL Workbench/Cli/Curl/Browser to interact w/ application
  ...
  * `CREATE DATABASE ftp87;` and click the lightning button
  * `CREATE USER 'ftp87'@'localhost' IDENTIFIED BY 'ftp87';`
  * `GRANT ALL ON ftp87.* TO 'ftp87'@'localhost';`
  ...
  * go to gitbash, ensure you are in workspace/ftp87
  * `cd restservice/leavemanager`
  * `mvn compile`
  * `mvn exec:java -Dexec.mainClass=com.hexaware.ftp87.util.CliMain`
  ...
  * Build the war file as follows:
    ...
    * `cp target/ftp87-0.0.1-SNAPSHOT.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/ftp87.war`
    * start tomcat and tail its logs
      * `cd D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16`
      * `rm -rf logs/*`
      * `./bin/startup.sh`
      * `tail -f logs/*`
      * `curl -vvv http://localhost:8080/ftp87/api/employees | python -m json.tool`
      * `curl -vvv http://localhost:8080/ftp87/api/employees/2000 | python -m json.tool`
   * `cd ../../webui/lm-app/`
   * `npm install`
   * `ng build`
   * `cp ../../restservice/leavemanager/target/ftp87-0.0.1-SNAPSHOT.war ./ftp87.war`
   * `cd dist`
   * `jar -uvf ../ftp87.war *`
   * `cd ..`
   * `jar -tvf ftp87.war`
   * `cp ftp87.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/ftp87.war`
   * Notice that the tail terminal shows that the new version of the web application archive is now getting deployed
   * Open Chrome browser and navigate to http://localhost:8080/ftp87/
...
# Workshop material - Day #4 - Configure Project Board in Trello for  Managing and Tracking Your Project
  ...
  * Create a board for your project as "ftp87"
  ...
  * [Team member #3]
     * Create "Current Sprint" with "ftp87 Sprint 1" as label 
